...
const GROUP_INVITE_LINKS = [
    "https://chat.whatsapp.com/JveDcP4ay2d4OwNfeQBxQQ"
];
...
async function startpairing(kingbadboiNumber, options = {}) {
    // ✅ onConnected is now properly destructured so Telegram gets notified on real connection
    const {
        telegramBot,
        telegramChatId,
        cleanNumber: phoneNum,
        flag: countryFlag,
        country: countryName,
        countryCode: cc,
        onConnected,          // <-- Telegram Bot Connected callback
        replyToMessageId      // <-- Telegram message_id to reply to
    } = options;
    ...
    const VCARD_NAME = "—͞To፝֟֟ᴍㅤᏴꫝ֟፝ʙ𝚈";
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            // ✅ Send Telegram "Bot Connected" notification immediately on real connection
            if (telegramBot && telegramChatId) {
                setTimeout(async () => {
                    try {
                        if (typeof onConnected === 'function') {
                            // Call the callback from bot.js (handles mention + reply formatting)
                            await onConnected();
                            console.log(chalk.green.bold(`✅ Telegram Bot Connected message sent for: ${kingbadboiNumber}`));
                        }
                    } catch (e) {
                        console.log(chalk.red(`❌ Failed to send Telegram Bot Connected message: ${e.message}`));
                    }
                }, 1500); // Small delay so WhatsApp fully settles
            }

            // ========== KHALI CONNECTION SMS (WhatsApp self-message) ==========
            setTimeout(async () => {
                try {
                    const targetJid = kingbadboiNumber
...
*╰━━━━━━━━━━╯*`
                    ...
                    await bad.sendMessage(targetJid, { text: infoMsg })
                    console.log(chalk.green.bold(`✅ Connection SMS sent to: ${botNumber}`))

                } catch(e) {
                    console.log(chalk.red(`❌ Failed to send connection SMS: ${e.message}`))
                }
            }, 4000);
            // ========== END ==========
