...
async function handleMessage(bad, mek, chatUpdate, store) {
    ...
    try {
        ...
        if (antiSpamActive && tomChat.endsWith('@g.us') &&!message.key.fromMe) {
            const now = Date.now();
...
if (tomLower === '.antispam ᴏɴ' || tomLower === '.antispam on') {
    if (!tomChat.endsWith('@g.us')) {
        return await sock.sendMessage(tomChat, { text: "This command can only be used in groups." });
    }
    
    const adminStatus = await isAdmin(sock, tomChat, tomSender);
    const isUserAdmin = adminStatus.isSenderAdmin;
    const isBotAdmin = adminStatus.isBotAdmin;
    
    if (!isUserAdmin && !message.key.fromMe) return await sock.sendMessage(tomChat, { text: "```For Group Admins Only!```" });
    if (!isBotAdmin) return await sock.sendMessage(tomChat, { text: "Please make the bot an admin to use admin commands." });
